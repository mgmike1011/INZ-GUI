...
LOGSystem::LOGSystem(QObject *parent, const QString &filepath): QObject{parent}, m_file(filepath)
{
    if (m_file.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Append)){
        ...
        this->m_TextMessage = this->m_DateTime.toString(Qt::DateFormat::ISODate);
        stream << m_TextMessage << "\n";
